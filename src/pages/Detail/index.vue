          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(spuSaleAttr) in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                <dd v-for="(spuSaleAttrValue) in spuSaleAttr.spuSaleAttrValueList" :key="spuSaleAttrValue.id"
                  changepirce="0"
            ...
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model.number="skuSum" @change="changeSkuNumber"/>
                <a href="javascript:" class="plus" @click="skuSum++">+</a>
                <a href="javascript:" class="mins" @click="skuSum > 1 ? skuSum-- : (skuSum = 1)">-</a>
              </div>
              <div class="add">
...
export default {
  ...
  methods:{
    ...
    changeSkuNumber(){
      // 用户输入进来的文本
      if (isNaN(this.skuSum) || this.skuSum < 1) {
        this.skuSum = 1;
      } else {
        this.skuSum = parseInt(this.skuSum);
      }
    },
    // 加入购物车大的回调函数
    async addShopCar(){
      ...
      try {
        ...
        // 2.服务器存储成功 ---- 进行路由的跳转并传递参数
        // 进行路由的跳转
        // 将产品的信息带给下一级的路由组件
        // 将传递的对象进行会话存储
        sessionStorage.setItem("SKUINFO",JSON.stringify(this.skuInfo))
        this.$router.push({name:'addcartsuccess',query:{skuNum:this.skuSum}})
      } catch (error) {
        alert(error)
      }
      // 2.服务器存储成功 ---- 进行路由的跳转并传递参数
