      ...
      <div class="py-container">
        ...
        <div class="bread">
            ...
            <ul class="fl sui-tag">
                ...
                <!-- 平台的售卖的属性值的展示 -->
                <li class="with-x" v-for="(attrValue, index) in searchParams.props" :key="index" v-show="attrValue">
                {{ attrValue.split(":")[1] }}
...
export default {
  ...
  // 因为需要切换到search组件，切换的时候需要将传递的参数传递到相应的组件之中
  beforeMount(){
  ...
  methods:{
    ...
    removeCategoryName(){ 
      ...
      // 严谨：本事是删除query，如果路径当中出现params不应该删除，路由跳转的时候应该带着
    ...
    // 删除关键字
    removeKeyword(){
      // 给服务器带的参数searchParams的keyworld置空
  ...
  watch:{
    // 监听路由的信息是否发生变化，如果发生变化，再次发起请求【既地址栏上的路由路径是否发生变化】
    $route(newValue,oldValue){
      // 再次发请求之前整理带给服务器参数
      Object.assign(this.searchParams, this.$route.query, this.$route.params);
      // 再次发起ajax请求
      this.getData()
      // 每一次请求完毕,应该把相应的1，2，3级分类的id置空，让他接受下一个相应的id
      this.searchParams.categoryId1 = undefined
      this.searchParams.categoryId2 = undefined
      this.searchParams.categoryId3 = undefined
